...
private final class FanService: @unchecked Sendable {
    ...
    private let timerQueue = DispatchQueue(label: "as.kargn.hottarget.helper.temperature-control")
    ...
    private func reconcile() {
        locked {
            ...
            do {
                ...
                NSLog("hottarget-helper fan control failed: \(error)")
                activeControl = nil
                cancelControlTimerLocked()
                do {
                    try reader.restoreAutomatic()
                } catch {
                    NSLog("hottarget-helper automatic reset failed: \(error)")
                }
            }
        }
    }
    ...
    private func reconcileLocked(_ control: FanControl) throws {
        ...
        case let .noise(mode, hotThreshold):
            let effectiveTemperature = stabilizer.effectiveTemperature(for: controlTemperature)
...
private final class ListenerDelegate: NSObject, NSXPCListenerDelegate {
    ...
    private func removeConnection(_ identifier: ObjectIdentifier) {
        ...
        NSLog("hottarget-helper disconnect reset failed: \(error)")
        }
    }
}

guard geteuid() == 0 else {
    NSLog("hottarget-helper must run as root")
    exit(77)
}
...
do {
    ...
    NSLog("hottarget-helper cannot establish signed client requirement: \(error)")
    exit(78)
}
...
let signalQueue = DispatchQueue(label: "as.kargn.hottarget.helper.signal")
